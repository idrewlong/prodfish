...
import { describe, it, expect } from 'vitest';
import { MONUMENT_SPOTS, minWorkPathClearance } from '../src/world/monuments.js';
...
describe('MONUMENT_SPOTS', () => {
  ...
  it('places stones out along the detour, not on the direct path', () => {
    for (const [x] of MONUMENT_SPOTS) expect(x).toBeGreaterThan(2);
  });
  it('flanks both sides of the route', () => {
    const rowMidX = 11;
    expect(MONUMENT_SPOTS.some(([x]) => x < rowMidX)).toBe(true);
    expect(MONUMENT_SPOTS.some(([x]) => x > rowMidX)).toBe(true);
  });
...
// Smallest horizontal distance from any monument to the scenic route.
export function minWorkPathClearance() {
  let min = Infinity;
  for (const [x, z] of MONUMENT_SPOTS) {
    for (let i = 0; i <= 300; i++) {
      const p = positionAt(i / 300, 'work');
...
function buildCrypt(scene) {
  ...
  group.position.set(15.5, 0, 6.2);
  group.rotation.y = -0.8;
  scene.add(group);

  return { group, anchor: new THREE.Vector3(15.5, 3.1, 6.2) };
}
